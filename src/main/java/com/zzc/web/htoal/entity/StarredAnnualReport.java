/**
 * AnnualReport.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis 1.4 Apr 22, 2006 (06:55:48 PDT) WSDL2Java emitter.
 */

package com.zzc.web.htoal.entity;

public class StarredAnnualReport  implements java.io.Serializable {
    private String otherIncome;

    private String shenHeDateTime;

    private String isStop;

    private String forHire;

    private String actualRent;

    private java.lang.String city;

    private java.lang.String nian;

    private java.lang.String code;

    private String tableDate;

    private java.lang.String unitName;

    private String fillDate;

    private java.lang.String unitMaster;

    private java.lang.String filler;

    private java.lang.String fillerTel;

    private String currentAssetsTotal;

    private String deposit;

    private String longTermInvest;

    private String fixAssetsTotal;

    private String fixAssetsInitialPrice;

    private String accumulatedDepreciation;

    private String currentYearDepreciation;

    private String capitalTotal;

    private String totalLiabilities;

    private String ownerBenefitTotal;

    private String realCapital;

    private String nationalCapital;

    private String collectiveCapital;

    private String personalCapital;

    private String corporateCapital;

    private String HMTCapital;

    private String foreignerCapital;

    private String roomRevenue;

    private String cateringIncome;

    private String operationIncome;
    
    private String operationCost;

    private String operationFee;

    private String operationTaxAddition;

    private String businessProfit;

    private String managementFee;

    private String tax;

    private String financeFee;

    private String StringerestPay;

    private String operationProfit;

    private String profitTotal;

    private String incomeTax;

    private String annualPayTotal;

    private String yearendEmployment;

    private String juniorEmployment;

    private String room;

    private String bed;

    private String nonIncome;

    private String subsidies;

    private String energyCosts;

    private String travelExpenses;

    private String costSales;

    private String gains;

    private String returnInvestment;

    private String theVAT;

    private String turnoverRate;

    private java.lang.String whetherPerform;

    private String loss;

	public String getOtherIncome() {
		return otherIncome;
	}

	public void setOtherIncome(String otherIncome) {
		this.otherIncome = otherIncome;
	}

	public String getShenHeDateTime() {
		return shenHeDateTime;
	}

	public void setShenHeDateTime(String shenHeDateTime) {
		this.shenHeDateTime = shenHeDateTime;
	}

	public String getIsStop() {
		return isStop;
	}

	public void setIsStop(String isStop) {
		this.isStop = isStop;
	}

	public String getForHire() {
		return forHire;
	}

	public void setForHire(String forHire) {
		this.forHire = forHire;
	}

	public String getActualRent() {
		return actualRent;
	}

	public void setActualRent(String actualRent) {
		this.actualRent = actualRent;
	}

	public java.lang.String getCity() {
		return city;
	}

	public void setCity(java.lang.String city) {
		this.city = city;
	}

	public java.lang.String getNian() {
		return nian;
	}

	public void setNian(java.lang.String nian) {
		this.nian = nian;
	}

	public java.lang.String getCode() {
		return code;
	}

	public void setCode(java.lang.String code) {
		this.code = code;
	}

	public String getTableDate() {
		return tableDate;
	}

	public void setTableDate(String tableDate) {
		this.tableDate = tableDate;
	}

	public java.lang.String getUnitName() {
		return unitName;
	}

	public void setUnitName(java.lang.String unitName) {
		this.unitName = unitName;
	}

	public String getFillDate() {
		return fillDate;
	}

	public void setFillDate(String fillDate) {
		this.fillDate = fillDate;
	}

	public java.lang.String getUnitMaster() {
		return unitMaster;
	}

	public void setUnitMaster(java.lang.String unitMaster) {
		this.unitMaster = unitMaster;
	}

	public java.lang.String getFiller() {
		return filler;
	}

	public void setFiller(java.lang.String filler) {
		this.filler = filler;
	}

	public java.lang.String getFillerTel() {
		return fillerTel;
	}

	public void setFillerTel(java.lang.String fillerTel) {
		this.fillerTel = fillerTel;
	}

	public String getCurrentAssetsTotal() {
		return currentAssetsTotal;
	}

	public void setCurrentAssetsTotal(String currentAssetsTotal) {
		this.currentAssetsTotal = currentAssetsTotal;
	}

	public String getDeposit() {
		return deposit;
	}

	public void setDeposit(String deposit) {
		this.deposit = deposit;
	}

	public String getLongTermInvest() {
		return longTermInvest;
	}

	public void setLongTermInvest(String longTermInvest) {
		this.longTermInvest = longTermInvest;
	}

	public String getFixAssetsTotal() {
		return fixAssetsTotal;
	}

	public void setFixAssetsTotal(String fixAssetsTotal) {
		this.fixAssetsTotal = fixAssetsTotal;
	}

	public String getFixAssetsInitialPrice() {
		return fixAssetsInitialPrice;
	}

	public void setFixAssetsInitialPrice(String fixAssetsInitialPrice) {
		this.fixAssetsInitialPrice = fixAssetsInitialPrice;
	}

	public String getAccumulatedDepreciation() {
		return accumulatedDepreciation;
	}

	public void setAccumulatedDepreciation(String accumulatedDepreciation) {
		this.accumulatedDepreciation = accumulatedDepreciation;
	}

	public String getCurrentYearDepreciation() {
		return currentYearDepreciation;
	}

	public void setCurrentYearDepreciation(String currentYearDepreciation) {
		this.currentYearDepreciation = currentYearDepreciation;
	}

	public String getCapitalTotal() {
		return capitalTotal;
	}

	public void setCapitalTotal(String capitalTotal) {
		this.capitalTotal = capitalTotal;
	}

	public String getTotalLiabilities() {
		return totalLiabilities;
	}

	public void setTotalLiabilities(String totalLiabilities) {
		this.totalLiabilities = totalLiabilities;
	}

	public String getOwnerBenefitTotal() {
		return ownerBenefitTotal;
	}

	public void setOwnerBenefitTotal(String ownerBenefitTotal) {
		this.ownerBenefitTotal = ownerBenefitTotal;
	}

	public String getRealCapital() {
		return realCapital;
	}

	public void setRealCapital(String realCapital) {
		this.realCapital = realCapital;
	}

	public String getNationalCapital() {
		return nationalCapital;
	}

	public void setNationalCapital(String nationalCapital) {
		this.nationalCapital = nationalCapital;
	}

	public String getCollectiveCapital() {
		return collectiveCapital;
	}

	public void setCollectiveCapital(String collectiveCapital) {
		this.collectiveCapital = collectiveCapital;
	}

	public String getPersonalCapital() {
		return personalCapital;
	}

	public void setPersonalCapital(String personalCapital) {
		this.personalCapital = personalCapital;
	}

	public String getCorporateCapital() {
		return corporateCapital;
	}

	public void setCorporateCapital(String corporateCapital) {
		this.corporateCapital = corporateCapital;
	}

	public String getHMTCapital() {
		return HMTCapital;
	}

	public void setHMTCapital(String hMTCapital) {
		HMTCapital = hMTCapital;
	}

	public String getForeignerCapital() {
		return foreignerCapital;
	}

	public void setForeignerCapital(String foreignerCapital) {
		this.foreignerCapital = foreignerCapital;
	}

	public String getRoomRevenue() {
		return roomRevenue;
	}

	public void setRoomRevenue(String roomRevenue) {
		this.roomRevenue = roomRevenue;
	}

	public String getCateringIncome() {
		return cateringIncome;
	}

	public void setCateringIncome(String cateringIncome) {
		this.cateringIncome = cateringIncome;
	}

	public String getOperationIncome() {
		return operationIncome;
	}

	public void setOperationIncome(String operationIncome) {
		this.operationIncome = operationIncome;
	}

	public String getOperationCost() {
		return operationCost;
	}

	public void setOperationCost(String operationCost) {
		this.operationCost = operationCost;
	}

	public String getOperationFee() {
		return operationFee;
	}

	public void setOperationFee(String operationFee) {
		this.operationFee = operationFee;
	}

	public String getOperationTaxAddition() {
		return operationTaxAddition;
	}

	public void setOperationTaxAddition(String operationTaxAddition) {
		this.operationTaxAddition = operationTaxAddition;
	}

	public String getBusinessProfit() {
		return businessProfit;
	}

	public void setBusinessProfit(String businessProfit) {
		this.businessProfit = businessProfit;
	}

	public String getManagementFee() {
		return managementFee;
	}

	public void setManagementFee(String managementFee) {
		this.managementFee = managementFee;
	}

	public String getTax() {
		return tax;
	}

	public void setTax(String tax) {
		this.tax = tax;
	}

	public String getFinanceFee() {
		return financeFee;
	}

	public void setFinanceFee(String financeFee) {
		this.financeFee = financeFee;
	}

	public String getStringerestPay() {
		return StringerestPay;
	}

	public void setStringerestPay(String stringerestPay) {
		StringerestPay = stringerestPay;
	}

	public String getOperationProfit() {
		return operationProfit;
	}

	public void setOperationProfit(String operationProfit) {
		this.operationProfit = operationProfit;
	}

	public String getProfitTotal() {
		return profitTotal;
	}

	public void setProfitTotal(String profitTotal) {
		this.profitTotal = profitTotal;
	}

	public String getIncomeTax() {
		return incomeTax;
	}

	public void setIncomeTax(String incomeTax) {
		this.incomeTax = incomeTax;
	}

	public String getAnnualPayTotal() {
		return annualPayTotal;
	}

	public void setAnnualPayTotal(String annualPayTotal) {
		this.annualPayTotal = annualPayTotal;
	}

	public String getYearendEmployment() {
		return yearendEmployment;
	}

	public void setYearendEmployment(String yearendEmployment) {
		this.yearendEmployment = yearendEmployment;
	}

	public String getJuniorEmployment() {
		return juniorEmployment;
	}

	public void setJuniorEmployment(String juniorEmployment) {
		this.juniorEmployment = juniorEmployment;
	}

	public String getRoom() {
		return room;
	}

	public void setRoom(String room) {
		this.room = room;
	}

	public String getBed() {
		return bed;
	}

	public void setBed(String bed) {
		this.bed = bed;
	}

	public String getNonIncome() {
		return nonIncome;
	}

	public void setNonIncome(String nonIncome) {
		this.nonIncome = nonIncome;
	}

	public String getSubsidies() {
		return subsidies;
	}

	public void setSubsidies(String subsidies) {
		this.subsidies = subsidies;
	}

	public String getEnergyCosts() {
		return energyCosts;
	}

	public void setEnergyCosts(String energyCosts) {
		this.energyCosts = energyCosts;
	}

	public String getTravelExpenses() {
		return travelExpenses;
	}

	public void setTravelExpenses(String travelExpenses) {
		this.travelExpenses = travelExpenses;
	}

	public String getCostSales() {
		return costSales;
	}

	public void setCostSales(String costSales) {
		this.costSales = costSales;
	}

	public String getGains() {
		return gains;
	}

	public void setGains(String gains) {
		this.gains = gains;
	}

	public String getReturnInvestment() {
		return returnInvestment;
	}

	public void setReturnInvestment(String returnInvestment) {
		this.returnInvestment = returnInvestment;
	}

	public String getTheVAT() {
		return theVAT;
	}

	public void setTheVAT(String theVAT) {
		this.theVAT = theVAT;
	}

	public String getTurnoverRate() {
		return turnoverRate;
	}

	public void setTurnoverRate(String turnoverRate) {
		this.turnoverRate = turnoverRate;
	}

	public java.lang.String getWhetherPerform() {
		return whetherPerform;
	}

	public void setWhetherPerform(java.lang.String whetherPerform) {
		this.whetherPerform = whetherPerform;
	}

	public String getLoss() {
		return loss;
	}

	public void setLoss(String loss) {
		this.loss = loss;
	}
    
    

}
