(function($, win, undef) {
	...
	var tipmsg = {
		...
		w : {
			...
			"p" : "请填写邮政编码！",
			"m" : "请填写手机号码！",
			"c" : "请填写电话号码0898-XXXXXXX格式！",
			"e" : "邮箱地址格式不对！",
			"url" : "请填写网址！",
			"w1" : "必须输入字母开头、可带数字、下划线的字符",
			"xs":"请输入9999999.9999以下的数字(小数最多四位)",
			"decimals" : "请输入正确的小数数值",
			"decimals1" : "请输入正确的1位小数数值",
			"decimals2" : "请输入正确的2位小数数值",
			"decimals3" : "请输入正确的3位小数数值",
			"decimals4" : "请输入正确的4位小数数值",
			"decimals5" : "请输入正确的5位小数数值",
			"mc" : "请输入正确的电话号码或者手机号码"
		},
		def : "请填写正确信息！",
		undef : "datatype未定义！",
	...
	Validform.util = {
		dataType : {
			...
			"decimals4" : /^(\-|\+)?\d+(\.\d{1,4})?$/,
			"decimals4" : /^(\-|\+)?\d+(\.\d{1,5})?$/,
			"mc"  :  /^((((0\d{2,3})-)?(\d{7,8})(-(\d{3,}))?)|(13[0-9]{9}$|14[0-9]{9}|15[0-9]{9}$|17[0-9]{9}|18[0-9]{9}))$/
		},
		...
		cssctl : function(obj, status) {
			switch (status) {
			case 1:
				obj.removeClass("Validform_wrong Validform_loading");
			...
			case 4:
				...
				break;
			default:
				obj.removeClass("Validform_right Validform_loading");
	...
	function creatMsgbox() {
		...
		msgobj = $(
				'<div id="Validform_msg"><div class="Validform_title">' + tipmsg.tit + '<a class="Validform_close" href="javascript:void(0);" style="color:black;">&chi;</a></div><div class="Validform_info"></div><div class="iframe"><iframe frameborder="0" scrolling="no" height="100%" width="100%"></iframe></div></div>')
				.appendTo("body");
