...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding: 1px;">
		<t:datagrid name="userList2" title="" queryMode="group"
			actionUrl="hotelController.do?starDatagridSta" idField="id" fit="true">
			...
			<t:dgCol title="国家系统审核状态" field="countryStatus" query="false"
				width="7" replace="<font color=\"red\">未填报</font>_1,<font color=\"#d2980c\">未审核</font>_2,已审核(审核不通过)_3,已审核(审核通过)_4"   ></t:dgCol>
			...
			<t:dgFunOpt funname="edit(districtStatus,status,provinceStatus,countryStatus,fillDate)" title="编辑"></t:dgFunOpt>
...
// 编辑
function edit(districtStatus, status, provinceStatus, countryStatus, fillDate){
	if(fillDate != null && fillDate.length != 0){
		var year = fillDate.substring(0,4);
		var nowYear = new Date().getFullYear();
		if(year >= nowYear){
			if(countryStatus == "3" || provinceStatus == "3" || status == "1" || status == "3" || districtStatus == "3"){
				createwindow("星级酒店基本信息年报编辑 ","hotelController.do?add", 600, 480);
			}else{
				alert("今年基本信息年报已填报");
			}
			return;
