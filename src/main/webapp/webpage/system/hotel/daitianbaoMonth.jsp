...
<body style="" scroll="yes">
	...
	<t:formvalid formid="saveHotelMonthly" refresh="false" dialog="true"
		tiptype="1" callback="back" action="hotelMonthlyController.do?saveDai"
		usePlugin="password" layout="table" beforeSubmit="addSub()">

		<input id="id" name="id" type="hidden" value="${hotelMonthly.id }">
		<input name="hotelId" type="hidden" value="${hotelmanage.id }">
...
$('.rent').change(function(){
	...
	$("input[name='rentPercent']").val(isNaN(parseFloat(rentPercent, 10).toFixed(2)) ? null : parseFloat(rentPercent, 10).toFixed(2));
});
function addSub(){
