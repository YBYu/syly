...
<table style=" font-size:12px;" cellpadding="0" cellspacing="1" class="formtable">
		<tbody>
			...
			<tr>
                ...
                <%-- <input disabled="disabled"  value="<fmt:formatDate value="${golfMonthly.dates }" type="date"/>" type="text"  pattern="yyyy-MM" datatype="*" "></td> --%>
        	...
        	<tr>
                ...
                <td class="value" width="16%" colspan="2"><input disabled="disabled"  value="${golfMonthly.otherTravelInfo.city }" type="text" datatype="*" "></td>
                <td align="right" width="16%">所在区 </td>
                <td class="value" width="30%">
                <input disabled="disabled"  value="<c:if test="${golfMonthly.otherTravelInfo.area==0 }">市辖区</c:if><c:if test="${golfMonthly.otherTravelInfo.area==1 }">崖州区</c:if><c:if test="${golfMonthly.otherTravelInfo.area==2 }">海棠区</c:if><c:if test="${golfMonthly.otherTravelInfo.area==3 }">天涯区</c:if><c:if test="${golfMonthly.otherTravelInfo.area==4 }">吉阳区</c:if>" type="text" datatype="*" ">
                </td>
            </tr>
            <!--法人信息-->
            <tr>
				<td align="right" width="16%">联系电话<span class="filedzt"></span></td>
				<td class="value" width="16%" colspan="2"><input disabled="disabled" value="${golfMonthly.otherTravelInfo.phone}" type="text" datatype="*" ></td>
                <td align="right" width="16%">邮政编码<span class="filedzt"></span></td>
                <td class="value" width="30%"><input disabled="disabled" value="${golfMonthly.otherTravelInfo.postalcode}" datatype="*" type="text"  ></td>
			</tr>
            <!--英文名称-->
            <tr>
				<td align="right" width="16%"><span class="filedzt">接待人次（单位：人次）</span></td>
				<td class="value" width="16%" colspan="2"><input disabled="disabled" value="${golfMonthly.receptionnum}" type="text"datatype="*" name="receptionnum "></td>
                <td align="right" width="16%"><span class="filedzt">营业收入（万元）</span></td>
                <td class="value" width="30%"><input disabled="disabled" value="${golfMonthly.businessincome}" type="text" datatype="*" name="businessincome"></td>
			</tr>     
			 <tr>
