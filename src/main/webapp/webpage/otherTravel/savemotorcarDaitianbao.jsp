...
<t:formvalid formid="saveot"   action="motorcarMonthlyController.do?savcmdais" callback="reloadTable" dialog="true"  layout="table">
...
<table style=" font-size:12px;" cellpadding="0" cellspacing="1" class="formtable">
		...
		<c:forEach var="item"  items="${listss}" varStatus="status">
			...
			<tr>
				...
				<td class="value" width="16%" colspan="2"><input   type="text" datatype="n1-9" errormsg="只能为1-9位整数"name="innum${status.count }"></td>
                <td align="right" width="16%"><span class="filedzt">出站人数（万人次）</span></td>
                <td class="value" width="30%"><input   type="text" datatype="/^\d{1,9}(.\d{1,2})?$/"   errormsg="请输入1-9位数字(最多两位小数)" " name="outnum${status.count }"></td>
			</tr>
			<tr>
            	<td align="right" width="16%">吞吐量（万人次）</td>
                <td class="value" width="16%" colspan="4"><input   name="throughput${status.count }" datatype="/^\d{1,9}(.\d{1,2})?$/"   errormsg="请输入1-9位数字(最多两位小数)"  type="text"      ></td>
