...
<t:formvalid formid="saveot"   action="flightMonthlyController.do?savfmdais" callback="reloadTable" dialog="true"  layout="table">
...
<table style=" font-size:12px;" cellpadding="0" cellspacing="1" class="formtable">
		...
		<c:forEach var="item"  items="${listss}" varStatus="status">
			...
			<tr>
				...
				<td class="value" width="16%" colspan="2"><input  type="text" datatype="n1-9" errormsg="只能为1-9位整数"name="receptionnum${status.count }"></td>
                <td align="right" width="16%"><span class="filedzt">营业收入（万元）</span></td>
                <td class="value" width="30%"><input  type="text" datatype="/^\d{1,9}(.\d{1,2})?$/"   errormsg="请输入1-9位数字(最多两位小数)"  name="businessincome${status.count }"></td>
			</tr>
			<tr>
            ...
            <tr>
                ...
                <td class="value" width="16%" colspan="4"><input  name="flighttimes${status.count }" type="text"  datatype="n1-9" errormsg="只能为1-9位整数"   ></td>
            </tr>
		</c:forEach>
