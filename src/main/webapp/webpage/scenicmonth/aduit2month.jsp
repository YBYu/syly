...
<body style="" scroll="">
<t:formvalid formid="formobj" refresh="true" dialog="true" action="scenicMonthController.do?save" usePlugin="password" layout="table" tiptype="4">
	...
	<table style="width:650px; font-size:12px;" cellpadding="0" cellspacing="1" class="formtable">
		<tbody>
            ...
            <tr>
				...
				<td class="value" width="16%" colspan="2"><input type="text" value="${monthdata.businesstax}" name="businesstax"   datatype="xs" class="notnecessary" /></td>
                <td align="right" width="16%"><span class="filedzt">利润总额（万元）</span></td>
                <td class="value" width="30%"><input type="text" value="${monthdata.totalprofit}" name="totalprofit"   datatype="decimals4"   class="notnecessary"></td>
			</tr>
            <!--法人代表、行政区域代码-->
            <tr>
                ...
                <td class="value" width="30%"><input type="text" value="${monthdata.laidworker}" name="laidworker"  class="notnecessary" datatype="n1-7" errormsg="只能为1-7位整数"></td>
			</tr>
            <!--所在地区、详细地址-->
            <tr>
            ...
            <tr>
				...
				<td class="value" width="16%" colspan="2"><input type="text" name="principal" value="<c:if test='${empty monthdata.principal}'>${scenicdata.chargename }</c:if><c:if test='${not empty monthdata.principal}'>${monthdata.principal }</c:if>" class="responsible" datatype="*" errormsg="请输入正确单位负责人" /></td>
                <td align="right" width="16%"><span class="filedzt">填表人</span></td>
                <td class="value" width="30%">
                	<input type="text" value="<c:if test='${empty monthdata.preparer}'>${scenicdata.informantname }</c:if><c:if test='${not empty monthdata.preparer}'>${monthdata.preparer }</c:if>" name="preparer" class="preparer" datatype="*" errormsg="请输入正确邮政编码">
                </td>
			</tr>
