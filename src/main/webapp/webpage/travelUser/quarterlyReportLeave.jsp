...
<t:datagrid name="leaveGrid" title="" queryMode="group" actionUrl="travelQuarterController.do?leaveGridOfUser" idField="id" fit="true">
<t:dgCol title="编号" field="id" hidden="true"></t:dgCol>
	...
	<%-- <t:dgDelOpt title="删除" url="travelQuarterController.do?deleteLeaveReport&id={id}" exp="status#ne#4"/> --%>
...
createwindowbutton("新建出境报表","travelQuarterController.do?addOrEditLeaveReport",button,850,500);
}

// 编辑
function edit(){
	var selection = $("#leaveGrid").datagrid('getSelected');
	if(selection == null){
		alert("请选择一条数据!");
		return;
	}
...
createwindowbutton("编辑出境报表","travelQuarterController.do?addOrEditLeaveReport&id="+selection.id,button,850,500);
}

// 查看详情
function checkDetail(id){
	createdetailwindow("出境报表详情","travelQuarterController.do?addOrEditLeaveReport&id="+id,850,500);
}
...
function countryContent(id){
	createdetailwindow('国家系统审核意见','travelQuarterController.do?countryContent&id='+ id+"&type=2", 450, 160);
}
</script>
